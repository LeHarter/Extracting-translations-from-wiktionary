...
#wiktionary = "enwiktionary-20210420-pages-articles.xml"
wiktionary = sys.argv[1]

word_pattern = re.compile('<title>(.*?)</title>')
language_pattern = re.compile('==([A-Z].*?)==')
wanted_PoS = {"Adjective","Adverb","Noun","Verb"}
pos_pattern = re.compile("===([A-Z][a-z]+)===")
translation_pattern = re.compile("{{trans-top\|(.*)}}")
...
with codecs.open("articles.xml","w","utf-8") as xml:
    xml.write("<articles>\n")
    with codecs.open(wiktionary,"r","utf-8") as f:
        for line in f:
            if not readin:
