...
language_pattern = re.compile('{{-([a-z]{2,3})-}}')
wanted_PoS = {"nafnorð","atviksorð","lýsingarorð","sagnorð"}
pos_pattern = re.compile("{{-[a-z]+-(.*?)-}}")
translation_pattern = re.compile("{{þýðingar")
import sys

#file = "iswiktionary-20210501-pages-articles.xml"
file = sys.argv[1]
...
def articleformat_to_xml(text,word):
    ...
    for line in text:
        if level == 1: # no language found yet
            ...
            if x:
                #meaning = x.group(1)
                article += "\t\t\t\t\t<translation info=\""+"None"+"\">\n"#None instead of meaning
                level = 5
            else:
                x = pos_pattern.match(line)
                if x:
